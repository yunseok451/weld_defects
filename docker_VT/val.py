...
def run(
    ...
    s = ('%22s' + '%11s' * 7) % ('Class', 'Images', 'Instances', 'Mask(P', 'R',
                                  'mAP50', 'mAP50-95)','f1_score')
    ...
    LOGGER.info(pf % ('all', seen, nt.sum(), *metrics.mean_results(), f1_mean))
    if nt.sum() == 0:
   ...
   # # Print results per class
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(metrics.ap_class_index):
           LOGGER.info(pf % (names[c], seen, nt[c], *metrics.class_result(i), f1_scores[i]))
            # print(names[c], seen, nt[c], *metrics.class_result(i),f1_score.mean(i))
        weighted_sum = sum( n * f1 for n, f1 in zip(nt, f1_scores))
        total_nt = nt.sum()
        weighted_average = weighted_sum / total_nt
        formatted_number = f"{weighted_average:.3f}"
        print()
        print("                                                                          Average F1-score : " , formatted_number)
        print()

    # Print speeds
    t = tuple(x.t / seen * 1E3 for x in dt)  # speeds per imaged
    if not training:
    ...
    mp_mask, mr_mask, map50_mask, map_mask = metrics.mean_results()
    ...
    final_metric = mp_mask, mr_mask, map50_mask, map_mask
